...
/**
 *
 * Xiaomi MiMo translator (pay-as-you-go, OpenAI-compatible API).
 * https://mimo.mi.com/docs/zh-CN/quick-start/summary/first-api-call
 *
 */
class MiMoTranslator : LanguageGroupedTranslator() {
    ...
    private var url: String = "https://api.xiaomimimo.com/v1/chat/completions"
    ...
    /**
     * Deep-thinking switch. Defaults to false (disabled) for faster translation responses;
     * set to true in the config to enable the model's thinking step. MiMo V2.5 series are
     * hybrid-thinking models, so the "thinking" parameter is always safe to send.
     */
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("MiMoTranslator init failed, url is invalid: $url")
            return false
        }

        val fileApiKey = (configs["apiKey"] as String?)

        // Prefer the apiKey from config file; fall back to the MIMO_API_KEY environment variable.
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("MIMO_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("MiMoTranslator init failed, MiMo apiKey not found. Please set apiKey in MiMo.yaml or MIMO_API_KEY in environment.")
            return false
        }

        mConcurrent = (configs["concurrent"] as Int?) ?: 1
        // Read the optional deep-thinking switch; absent key keeps it disabled by default.
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("MiMoTranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("MiMoTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("MiMoTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            // Control MiMo deep-thinking mode; disabled by default for faster responses.
            ...
            logger.info("MiMoTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("MiMoTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("MiMoTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("MiMoTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "MiMoTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
